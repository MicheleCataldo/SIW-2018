...
class StudenteDaoJDBC implements StudenteDao {
	...
	public void save(Studente studente) {
		...
		try {
			String insert = "insert into studente(matricola, nome, cognome, data_nascita, data_iscrizione, indirizzo_codice) values (?,?,?,?,?)";
			...
			long secs_nascita = studente.getDataNascita().getTime();
			statement.setDate(4, new java.sql.Date(secs_nascita));
			long secs_iscrizione = studente.getDataIscrizione().getTime();
			statement.setDate(5, new java.sql.Date(secs_iscrizione));
			statement.setLong(6, studente.getIndirizzo().getCodice());
	...
	public Studente findByPrimaryKey(String matricola) {
		...
		try {
			...
			if (result.next()) {
				...
				long secs_nascita = result.getDate("data_nascita").getTime();
				studente.setDataNascita(new java.util.Date(secs_nascita));
				long secs_iscrizione = result.getDate("data_nascita").getTime();
				studente.setDataNascita(new java.util.Date(secs_iscrizione));
	...
	public void update(Studente studente) {
		Connection connection = this.dataSource.getConnection();
		try {
			String update = "update studente SET nome = ?, cognome = ?, data_nascita = ?, data_iscrizione = ?, indirizzo_codice = ? WHERE matricola=?";
			PreparedStatement statement = connection.prepareStatement(update);
			statement.setString(1, studente.getNome());
			statement.setString(2, studente.getCognome());
			long secs_nascita = studente.getDataNascita().getTime();
			statement.setDate(3, new java.sql.Date(secs_nascita));
			long secs_iscrizione = studente.getDataNascita().getTime();
			statement.setDate(4, new java.sql.Date(secs_iscrizione));
